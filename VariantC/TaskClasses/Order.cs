...
namespace VariantC.TaskClasses
{
    record Order //запись заказа
    {
        ...
        //}
        public void GetInformationAboutOrder() //Вывести полную информацию о заказе
        {
            Console.WriteLine($"Номер заказа: {_orderNumber}. Дата поступления: {_receiptDay}");
            Console.WriteLine("Товары в заказе: ");
            for (int i = 0; i < ProductsInOrder.Count; i++) // Перебирает все товары
            {
                Console.WriteLine($"{ProductsInOrder[i].ProductIn}. Количество товара: { ProductsInOrder[i].Amount}.");
